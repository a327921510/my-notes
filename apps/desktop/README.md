...
基于 [Electron](https://www.electronjs.org/) 的桌面端壳，复用 `@my-notes/web` 构建产物，支持 **Windows (.exe)**、**macOS (.dmg)** 与 Linux (AppImage) 打包。
...
| `pnpm dist:mac` | 一键产出 macOS `.dmg` 到 `release/`（须在 **macOS 主机** 上执行）；同样默认使用 npmmirror 镜像 |
| `pnpm dist:linux` | 产出 Linux AppImage（用于本地 / 容器内验证） |

> 注意：Windows 端 `.exe` 必须在 **Windows 主机** 或具备 `wine` 的 CI 环境上执行 `pnpm dist:win`；macOS 端 `.dmg` 必须在 **macOS 主机** 上执行 `pnpm dist:mac`；本仓库的 cloud agent / Linux 容器仅用于编译主进程脚本，最终安装包请在对应平台机器上打包。
>
...
>
> 若报 `cannot access ... app.asar ... being used by another process`：说明 `release\win-unpacked`（Windows）或 `release/mac`（macOS）被占用。请 **托盘退出 MyNotes**、关闭打开该目录的 Finder/资源管理器窗口，必要时结束 `MyNotes` 进程；仍失败时脚本会改输出到 `release-build-<时间戳>/`，或把旧目录重命名为 `*.locked-*`。占用解除后可手动删除整个 `apps/desktop/release` 再打包。
...
- **本地路径打开**：桌面版可在系统文件管理器中打开 Markdown/表格里的本地路径；Windows 使用资源管理器，macOS 使用 Finder（路径格式分别为盘符/UNC 与 `/`、`~/`）。
...
- 产出物：Windows 为 `MyNotes-Setup-*.exe`（NSIS 安装包）与 `MyNotes-Portable-*.exe`（免安装）；macOS 为 `MyNotes-*-x64.dmg` / `MyNotes-*-arm64.dmg`；二者均含完整 UI，区别仅为分发形式。
