...
export function useSitesState() {
  ...
  const cloneSite = useCallback(
    async (sourceSiteId: string, payload: { name: string; address: string }) => {
      const siteId = createId("site");
  ...
  const updateItem = useCallback(
    async (
      siteId: string,
      itemId: string,
      patch: Partial<Pick<SiteItem, "name" | "content">>,
    ) => {
  ...
  const removeItem = useCallback(async (siteId: string, itemId: string) => {
    const current = await db.site_items.get(itemId);
    if (!current || current.siteId !== siteId) return;
