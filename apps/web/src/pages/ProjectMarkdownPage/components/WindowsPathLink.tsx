...
import { isDesktopApp, openPathInExplorer } from "@/utils/desktopBridge";
...
export const WindowsPathLink = memo(function WindowsPathLink({
  ...
  const handleOpen = useCallback(() => {
    void (async () => {
      ...
      if (result.ok) {
        message.success(result.openedParent ? "已在资源管理器中打开所在文件夹" : "已在资源管理器中打开");
        return;
      }
  ...
  return (
    <button
      ...
      onClick={handleOpen}
      disabled={!canOpen}
      title={canOpen ? "在资源管理器中打开" : "请使用 MyNotes 桌面版打开本地路径"}
      aria-label={canOpen ? `在资源管理器中打开 ${path}` : `本地路径（仅桌面版可打开）：${path}`}
    >
