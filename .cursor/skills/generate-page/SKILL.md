...
前置条件：项目中已有 `.cursor/rules/page-layering.rule.mdc`（架构约束规则）和 `.cursor/rules/react-performance.rule.mdc`（性能约束规则）。

参考资料：
- 完整代码示例与层间通信模式：`references/page-layering-guide.md`
- memo / useCallback / useMemo 的完整决策与示例：`references/react-performance-guide.md`
...
> 通用职责约束已在 `page-layering.rule.mdc` 定义；性能要求（memo / useCallback / useMemo）已在 `react-performance.rule.mdc` 定义。下面只列**生成时的额外注意点**。

### 页面入口 (index.tsx)
- 导出命名组件 `export function <PageName>Page()` 并追加 `export default <PageName>Page`（路由 `lazy()` 需要默认导出）
- 调用业务 Hooks 获取 `state + actions`，持有跨区域共享状态
- JSX 中只做布局编排，复杂逻辑下沉到区域组件
- 跨区域联动在此中转（区域 A 的回调 → 更新共享状态 → 区域 B 通过 props 接收）

### 区域组件 (<Region>Panel.tsx)
- 导出命名组件，Props 类型紧跟在 import 后面 export
- 内部可调用本页 `hooks/` 下的业务 Hook，可拥有局部 UI 状态（折叠、排序、搜索词）
- 对外只通过 `onXxxChange` / `onRegisterActions` 等回调通信，不 import 其他区域组件的内部模块

### 业务 Hooks (use<Domain>.ts)
- 导出自定义 Hook 函数，返回结构：`{ data/state 相关, action 函数, loading?, error? }`
- 内部管理：API 调用、数据状态、loading/error、缓存、业务计算
- 不包含 JSX、不 import React 组件、不操作 DOM

### 纯展示组件 (<Widget>View.tsx)
- 导出方式：`export const XxxView = memo(function XxxView(...) { ... })`
- Props 类型紧跟在 import 后面 export
- 只使用 props，不调用 API / 数据库 / 全局 store
- 通过 `onXxx` props 向上报告事件

### 类型文件 (types.ts)
- 只放被 ≥2 个文件引用的页面级类型；单文件内部用的类型就近定义
...
> 性能相关（memo / useCallback / useMemo 的取舍）统一遵守 `react-performance.rule.mdc`，本 skill 不再重复罗列；遇到具体场景需要决策时查阅 `references/react-performance-guide.md`。
