...
### 3.5 components/ProjectCard.tsx（纯展示）
...
### 3.6 components/MemberAvatar.tsx（纯展示）
...
export function MemberAvatar({ member, onRemove }: MemberAvatarProps) {
  return (
...
### 3.7 components/EmptyPlaceholder.tsx（纯展示）
...
export function EmptyPlaceholder({ icon = "📋", title, description, action }: EmptyPlaceholderProps) {
  return (
